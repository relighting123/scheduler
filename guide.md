...
**CLI 예시 (강화학습 — 계획제품×공정×장비모델별 최적 대수 학습)**
```bash
# 학습 (모방학습 초기화 → PPO). 완료 후 scheduler_ppo_model.zip 저장
...
# 추론 (학습된 정책으로 24슬롯 시뮬 후 최종 대수·RTD_CONV 출력)
python3 run.py infer --timekey 20251020070000
python3 run.py infer   # RULE_TIMEKEY=DB MAX
